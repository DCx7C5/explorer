...
function get_summary(coin, exchange, cb) {
    ...
    request({uri: url, json: true}, function (error, response, body) {
        if (error) {
            ...
            summary['buy'] = parseFloat(body['ticker']['buy']).toFixed(8);
            summary['sell'] = parseFloat(body['ticker']['sell']).toFixed(8);
            summary['vol'] = parseFloat(body['ticker']['vol']).toFixed(8);
...
function get_trades(coin, exchange, cb) {
    var req_url = base_url + '/trades?market=' + coin.concat(exchange).toLowerCase();
...
function get_orders(coin, exchange, cb) {
    var req_url = base_url + '/order_book?market=' + coin.concat(exchange).toLowerCase();
    //console.log("send request to - " + req_url)
    request({ uri: req_url, json: true }, function (error, response, body) {
        if(error)
            return cb(error, null);
